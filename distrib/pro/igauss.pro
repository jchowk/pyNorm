;+
;				iGAUSD.PRO (adapted from IMGAUSS)
;				Version 1.0
;
;Program Description:
...
;
LOOP:
	PRINT,'iGAUSS::  ## GAUSSIAN FIT MODE##'
	PRINT,'iGAUSS::  Mark (C1)   Clear (C2)   Quit (C3)'
	...
	IF xpos2 LE xpos1 THEN BEGIN
		PRINT,'iGAUSS::  Please mark region again with x1 < x2.'
		PLOT,x,y & GOTO,LOOP	
	ENDIF
;
...
;
	PRINT,'iGAUSS::  Information Dump Follows'
	PRINT,'----------------------------------------'
	...
	PRINT,'---------------------------------------'
	PRINT,'iGAUSS::  End Information Dump'
;
;Wait for key to be hit.
;
	PRINT,'iGAUSS::  Hit any key to continue.'
	dummy = GET_KBRD(1)
	RETURN
;------------------------------------------------------------------------------
ESCAPE:
	PRINT,'iGAUSS::  '+!err_string
	RETURN  &  END
