...
;   	    	 dv90,v90a,v90b: 90% optical depth width and 5% and 95% velocity boundary
...
;       02/01/14  NL    - Version 1.1 - added batch option
;       02/03/14  NL    - Version 1.2 -  added dv90 interval; not always stable... 
;       02/05/15  NL    - Version 1.3 -  added mask option for profile fitting
;       12/05/15  NL    - Version 1.4 -  updated output error vector 
;------------------------------------------------------------------------------
PRO iNORM,v,f,ef=ef,wavc=wavc,root=root,ion=ion,fval=fval,gam=gam,object=object,redshift = redshift,batch=batch,vbatch1=vbatch1,vbatch2=vbatch2 
...
;------------------------------------------------------------------------------
BATCH:
;This section integrates N, EW, v, b plots over a given velocity range automatically
;If no continuum exists, then return to CONTROL if profile is not 
;normalized.
;
;If wavelength space has not been chosen then return to CONTROL
;
   if NOT keyword_set(vbatch1) then begin
   print, 'iNORM:: Cannot enter batch mode since no velocities are defined'
   GOTO,ERROR
   endif else begin 
      if NOT keyword_set(vbatch2) then begin
      vbatch2 = -vbatch1
   print, 'iNORM:: No end velocity defined'   
   print, 'iNORM:: Assuming v2 = -v1 for the integrationn!!!'   
      endif
   endelse
  ...
  if NOT keyword_set(ycon) then  ycon = y/y
  if NOT keyword_set(ycon_sig) then  ycon_sig = ycon * 0.           
	 coflag = 1
;
;Call iCOL_PREP to do the column density calculation.
;
	PRINT,'iNORM::  ##APPARENT COLUMN DENSITY MODE##'
	iCOL_PREP_BATCH,x,y,ey,ycon,sigma,ycon_sig,wavc,fval,vbatch1,vbatch2
	GOTO,QUIT
;------------------------------------------------------------------------------
LOOP15:
...
;------------------------------------------------------------------------------
LOOP21:
;This section allows a spectrum to be masked. 0 = good, 1 = bad
;
	PRINT,'iNORM::  ##MASK MODE##'
	iMask,x,y,ey,mask,maskflg
	GOTO,CONTROL
;------------------------------------------------------------------------------
ERROR:
...
; no error defined.
        ...
        fnorm  = y/ycon
        efnorm1 =  sqrt((ycon_sig/ycon)^2.0 + sigma0^2.0)
        efnorm2 =  (ycon_sig/ycon) + sigma0
	efnorm = (efnorm1 + efnorm2)/2
        ...
        cont = ycon 
        econt = ycon_sig
	dv90err = vaerr * sqrt(2)
	col2sig =alog10(10^col3sig/3.*2.)
	vaod1 = v1
	vaod2 = v2
	name = object
	vnorm= vel
...
;        help,root
       SAVE,file=root+'_o.save',v,f,ef,vel,vnorm,fnorm,efnorm,efnorm1,efnorm2,flux,eflux,cont,econt,$
       wavc,ion,fval,gam,object, redshift,vshft,snr,dv90,v90a,v90b,dv90err,gl,gb,ra,dec,z,name,$
       ncol, ncole1,ncole2,ncolez, w,w_es,w_ec,w_et,w_ez,va,vaerr, ba, baerr, m3, m3err,v1,v2,vaod1,vaod2,col3sig, col2sig,mask,mask_cont         
...
; QUIT for batch mode
  ...
  if  keyword_set(vbatch1) then begin
...
;        help,root
       ...
       ncol, ncole1,ncole2,ncolez, w,w_es,w_ec,w_et,w_ez,va,vaerr, ba, baerr, m3, m3err,v1,v2,vaod1,vaod2,col3sig, col2sig,mask,mask_cont        
        print,'iNORM:: Saving resuls and data in....   '+root+'_o.save' 
	PRINT,'iNORM::  Ending iNORM batch session...'
    	endif else begin 
        print,'iNORM:: No result to save...' 
	PRINT,'iNORM::  Ending iNORM batch session...'
	endelse
	endelse
