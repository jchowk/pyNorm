...
;
	; ON_IOERROR,ESCAPE
	; IF N_PARAMS() NE 7 THEN coflag = 0
;
;Print instructions.
;
	PRINT,'iCont::  Mark (C1)   Clear (C2)   Quit (C3)'
...
;
	...
	xpos2 = xpos2 < MAX(x)  &  !c = 0
  	PRINT,'iCont::  Right limit: ',xpos2
	OPLOT,[xpos2,xpos2],liney1,color=200
;
;Check to be sure range is not reversed.  If reversed, get again.
;
	IF xpos2 GT xpos1 THEN OPLOT,[xpos1,xpos2],[1,1]*0.9*!y.crange(1),color=200
