...
;
  a = double(TOTAL(tau*x*dx))
  m1 = double(a/tautot)
;
;Calculate second moment (width).
;
  b = double(TOTAL(tau*(x-m1)^2*dx))
  m2 = double(SQRT(b/tautot))
;
;Calculate third moment (skewness).
;
  f = double(TOTAL(tau*((x-m1)/m2)^3*dx))
  m3 = double(f / tautot)
;
;Calculate extent (same as m2, except that m1 is assumed to be 0).
;
  b4 = double(TOTAL(tau*(x-0.0)^2*dx))
  m4 = double(SQRT(b4/tautot))
